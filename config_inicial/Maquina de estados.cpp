...
// Parametros de la caminata
float walkSpeed = 1.2f;          
float walkHz = 3.0f;          
float frontAmp = 25.0f;         
float rearAmp = 25.0f;         
float tailAmp = 20.0f;        
float tailHz = 6.0f;          
float headAmp = 10.0f;         
...
// Avance del pero, se mueve en Z para avanzar 
const glm::vec3 dogForward(0.0f, 0.0f, 1.0f);
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Point light 1 (colores animados si Light1 != 0)
        glm::vec3 lightColor;
        ...
        // Pata delantera izquierda (eje -X)
        model = modelTemp;
...
void Animation() {
    ...
    // Perro caminando
    if (AnimDog) {
        // Avance independiente de FPS
        dogPos += dogForward * walkSpeed * deltaTime;
        ...
        // Paso senoidal
        const float phase = sinf(twoPi * walkHz * t);

        // Patas: delanteras en fase, traseras en contrafase
        FLegs = frontAmp * phase;   
        RLegs = -rearAmp * phase;  

        // Cola y cabeza
        tail = tailAmp * sinf(twoPi * tailHz * t);
        head = headAmp * sinf(twoPi * headHz * t);


    }
    else {
        // Relajación suave al detenerse
        FLegs *= 0.92f;
        RLegs *= 0.92f;
        tail *= 0.90f;
        head *= 0.90f;
...
void MouseCallback(GLFWwindow* window, double xPos, double yPos)
{
    ...
    GLfloat yOffset = lastY - yPos; // invertido porque Y va de abajo a arriba
